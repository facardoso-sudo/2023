...
void _subCallback_odom(const nav_msgs::Odometry::ConstPtr& _msg)
{
	...
	_x = _msg->pose.pose.position.x;
	_y = _msg->pose.pose.position.y;

	_delta = sqrt(pow(_x_ant-_x,2)+pow(_y_ant-_y,2));

	_travelled = _travelled + _delta;

	tf::poseMsgToTF(_msg->pose.pose, _pose);
...
int main(int argc, char **argv){
  ...
  ros::Subscriber _sub = _n.subscribe("v2_odom", 1000, _subCallback_odom);
...
/*** Regras de controle de desvio angular - end ***/
  ...
  while (ros::ok()){
    ...
    while ((abs(erroorie) > tolerance_orie) && (abs(_erroorie) > _tolerance_orie)) {
        ...
        msg.angular.z = fc_desvio_angular.make_inference(ang_obs, dist_obs);
				...
				printf("Distância total percorrida v1: %.2fm\n", travelled);
				printf("Erro de orientação v1: -\n");
				printf("Erro de posição v1: %.2f m\n",erropos);
				printf("Velocidade angular enviada v1: %.2f rad/s\n",msg.angular.z);
				printf("Velocidade linear enviada v1: %.2f m/s\n\n",msg.linear.x);
        ...
        //Controle de posição
          ...
          ROS_WARN("...Posicao alcancada...");
			...
			if(_is_obs){
				...
				system("clear");
				printf("Controle Ativo: DESVIO DE OBSTÁCULO.\n");
				printf("Distância total percorrida v2: %.2fm\n", _travelled);
				printf("Erro de orientação v2: -\n");
				printf("Erro de posição v2: %.2f m\n",_erropos);
				printf("Velocidade angular enviada v2: %.2f rad/s\n",_msg.angular.z);
				printf("Velocidade linear enviada v2: %.2f m/s\n\n",_msg.linear.x);

			}else{
			
				//Calcula o setpoint da orientação.
				_angulo = atan2(_posdesejada[1]-_y,_posdesejada[0]-_x);
		  	
		  		//Calcula a diferença entre os ângulos.
		  		_erroorie = (_angulo - _theta + 2*M_PI);

		  		//Calcula o MOD360.
		  		_erroorie = _erroorie/(2*M_PI) - 1;
		  		_erroorie = 2*M_PI*_erroorie;
		  		if (_erroorie < 0){
				  	_erroorie = _erroorie + 2*M_PI;
		  		}
				...
				if (abs(_erroorie) > _tolerance_orie){
					_ori_ok = false;
					_msg.angular.z = fc_angular.make_inference(_erroorie);
				}else{
					_ori_ok = true;
				}
				...
				printf("Distância total percorrida v2: %.2fm\n", _travelled);
				printf("Erro de orientação v2: %.2f rad\n",_erroorie);
				printf("Erro de posição v2: %.2f m\n",_erropos);
				printf("Velocidade angular enviada v2: %.2f rad/s\n",_msg.angular.z);
				printf("Velocidade linear enviada v2: %.2f m/s\n\n",_msg.linear.x);
    ...
    ROS_WARN("...Posicao alcancada R1...");
    msg.linear.x = 0;
    msg.angular.z = 0;  
    pub.publish(msg);
    ros::spinOnce();

	ROS_WARN("...Posicao alcancada R2...");
    _msg.linear.x = 0;
    _msg.angular.z = 0;  
    pub.publish(_msg);
    ros::spinOnce();
