...
int main(int argc, char **argv){
  ros::init(argc, argv, "pid_coppelia2");
 
  ros::NodeHandle n;
  ros::Publisher pub = n.advertise<geometry_msgs::Twist>("v2_cmd_vel", 1000);
  ros::Subscriber sub = n.subscribe("v2_odom", 1000, subCallback);
  ros::Subscriber sub_lidar = n.subscribe("v2_scan", 1000, subCallback_lidar);
...
/*** Regras de controle de desvio angular - end ***/
  ...
  while (ros::ok()){
    ...
    while ((abs(erroorie) > tolerance_orie)) {
        ...
        ROS_INFO("X2=%f  Y2=%f ",feedback.pose.pose.position.x,
                               feedback.pose.pose.position.y);
        
        ROS_INFO("LIN_X2 = %f     ANG_Z2 = %f", linearX, angularZ);
        ROS_INFO("----------------------");
